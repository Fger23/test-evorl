...
"""Main-compatible remote policy client used by recording loops.

This module intentionally preserves the protocol-v1 recording behavior from
``origin/main``. RTC recording support lives in ``recording_remote_policy`` and
is kept separate from the human-in-loop recording entry point.
"""
...
class RemotePolicyActionClient:
    """Small asynchronous protocol-v1 client for remote policy recording."""
