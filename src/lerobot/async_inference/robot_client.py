...
def _make_rtc_action_client(cfg: RobotClientConfig, robot: Robot):
    """Build the protocol-v2 RTC client used only by this module entrypoint."""
    ...
    remote_cfg = RemotePolicyRecordConfig(
        ...
        # RTC installs a single raw/processed pair atomically; overlap
...
def run_rtc_client(cfg: RobotClientConfig) -> None:
    ...
    try:
        ...
        action_client.start()
        # Reset after policy setup so the freshly loaded policy and the local
